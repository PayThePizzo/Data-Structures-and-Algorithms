...
2. All the edges in A **are safe and light edges** crossing the **cut (Q, {Q\V})**:
   1. **EXTRACT_MIN(Q)** is based on key[u] and **extracts** the vertex **u with the minimum-weight-edge in Q**.
      1. This implies, **there exists a light-edge (u,v) u∈Q, v∈Q\V : v∈ADJ[u]**
         1. The light edge (u,v) connects CC(u) to CC(v) in G(A).
3. By the end of the algorithm G(A) with A = {(u, π[u]) | u in V\{r}} is an MST
   * It is kept acyclic.
   * All the keys and predecessors are up-to-date thanks to `w(u,v)<d[v]`
