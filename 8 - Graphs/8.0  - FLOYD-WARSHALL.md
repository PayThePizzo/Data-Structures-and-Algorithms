...
* _D_^k-1(ij), the set of paths p(i,j) which **do not include** that node **k**, but only the previous ones 
...
* min(w(p), _Ď_^k(ij)):
  * _D_^k-1(ik), SP from i to k, with intermediate vertices xl<=k-1
  * _D_^k-1(kj), SP from k to j, with intermediate vertices xl<=k-1! 
