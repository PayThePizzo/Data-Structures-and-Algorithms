...
solution! We can represent recursively our sets.
...
* For the elementary principle we saw, d^k(ij) = min(w(p), p∈_D_^k(ij)) = **min{min(w(p), _Ď_ ^k(ij)), min(w(p), _D_ ^k-1(ij))}**
  * min(w(p), _D_^k-1(ij)) = d^k-1(ij), elements of the actual matrix, which are already available.
  * min(w(p), _Ď_^k(ij)), can be divided into:
    * _D_^k-1(ik), paths that go from i to k are simple!
    * _D_^k-1(kj), paths that go from k to j are simple! 
