...
### Demonstration
How do we find every d(ij) in D, starting from D^0?
We said that D^k(ij) contains all the simple SPs between i and j such that all their intermediate vertices are <= k.

Since d^k(ij) = min(w(p), p∈_D_^k(ij)), for _D k-limited property_, if we partition _D_^k(ij) into two halves and try 
to find the minimum, we would have the solution! 

We can represent recursively our sets: **_D_^k(ij) = _Ď_^k(ij) U _D_^k-1(ij)**
...
* min(w(p), _Ď_^k(ij)) can be decomposed into the union of two simple SPs (k only appears once), by _decomposition of distance_:
  * _D_^k-1(ik) = d^k-1(ik), SP from i to k, with intermediate vertices xl<=k-1
  * _D_^k-1(kj) = d^k-1(kj), SP from k to j, with intermediate vertices xl<=k-1 
