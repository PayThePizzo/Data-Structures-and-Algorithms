...
3) p must include an edge (x,y) with x∈S, y∈Q, that crosses the cut between S and Q (otherwise there is no path).
   ...
   4) d[u]<=d[y], if Dijkstra decides to extract u before y, by _construction of Dijkstra_
      1) Both u and y are in S
   5) However, δ(s,u)>=δ(s,y), by _hypothesis of positive weights_.
