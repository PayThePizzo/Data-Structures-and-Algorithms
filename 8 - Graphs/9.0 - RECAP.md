...
### Data Structures
* Structure of _predecessors pointing to their fathers_. Here we use a min-priority queue `Q` on `key[v]` (binary heap);
...
* Root vertex `r`;
* **RETURNS**:A tree candidate `A = {(u, π[u]) in E | u in V-{r}-Q}`, where the edges are saved implicitly
...
### Restrictions
* G(V,E,w) is either directed or undirected and an edge-weighted graph, with a real value function w
* We assume that w(u,v)>=0 ∀(u,v)∈E, all **edge weights are non-negative**

### Data Structures
* A min-priority queue `Q` on `d[u]` of vertices which have not been selected yet (binary heap or array).
  * `d[u]`, the estimate for the SP from s to u
...
* A source node `s`
* **RETURNS**: d, the vector containing the estimations and Gπ, predecessor subgraph
...
### Complexity
1. Array - Dense Graph, T(n) = <mark>**O(n^2)**</mark> =  Θ(n) + O(n^2) + O(m)
...
2. Bin. Heap - Sparse Graph, T(n) = <mark>**O(m * log(n))**</mark>
   1. Initialization: Θ(n)
...
### Data Structures
* INPUT: A matrix-like data structure W with w(ij) ∈ R(nxn) such that, w(ij) equals to:
  * **0**, if `i==j`, Main Diagonal
  * **weight w(i,j)**, if `i!=j AND (i,j)∈E`, Edge exists
  * **+inf()**, if `i!=j AND (i,j)∉E`, No edge exists
