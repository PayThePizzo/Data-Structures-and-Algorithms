...
### Data Structures
* INPUT: A **matrix-like data structure** representing the graph, `W` with w(ij) ∈ R(nxn) such that, w(ij) equals to:
  * **0**, if `i==j`, Main Diagonal
    * if `i==j < 0` we found a negative cycle.
  * **weight w(i,j)**, if `i!=j AND (i,j)∈E`, Edge exists
  * **+inf()**, if `i!=j AND (i,j)∉E`, No edge exists
* OUTPUT: A matrix D(ij) ∈ R(nxn)
  * When the algorithm stops, we expect **d^k(ij) = δ(i,j)**
  * 
