...
* The **set A**, **is a forest** G(A), trivially;
  * Initially empty.
* During each iteration, the **cut is represented by the extraction of the edge (u,v)** with **minimum weight** such that `find_set(u) != find_set(v)`. 
  * This ensures that the two vertices are respectively in S (set of vertices already extracted) and in V\S, but never in the same set.
  * Else we would have a cycle. 
* All the edges in G(A) are always light and safe for G(A), trivially.
* **G(A) becomes an MST by the end of the algorithm**, trivially.
  * G(A) is acyclic and the CCs are trees connected through the edges, trivially
