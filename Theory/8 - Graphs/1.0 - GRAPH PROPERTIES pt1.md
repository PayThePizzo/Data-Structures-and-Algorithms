...
## Induced Subgraph
A subgraph induced by a subset $V' \subseteq V$, $G\[ V' \]$, is a subgraph $G' = (V', E')$ such that:
* $V' \subseteq V$
* $E' = E \cap V' \times V'$
...
## Density
For a graph, the **density** is a real number $∈ \[ 0,1 \]$ given by the function $\delta (G) = \frac{|E|}{max(E)}$, 
with $max(E)$ as the maximum number of edges possibly existing for that particular graph. Let $n = |V|, m = |E|$
...
### Clique
Let G be a graph such that, $G=(V,E)$. A **clique** is a $\mathcal{C} \subseteq V$, a **subset of $V$ where all the couples of edges
are connected by an edge in E $\mathcal{C} = \{ \mathcal{C} \subseteq V | \forall u,v \in \mathcal{C} \rightarrow \exists (u,v) \in E$.
...
* $V = \{ V(L) \cup V(R) | V(L) \cap V(R) = \emptyset \}$
    * Dividing V in two subsets V-left and V-right, which have no common vertex.
* $G\[ V(L) \] = \emptyset, G\[ V(R) \] = \emptyset$ are empty
